...
func (c *tunnelServiceClient) Tunnel(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Frame, Frame], error) {
...
type TunnelService_TunnelClient = grpc.BidiStreamingClient[Frame, Frame]
...
func (UnimplementedTunnelServiceServer) Tunnel(grpc.BidiStreamingServer[Frame, Frame]) error {
...
func _TunnelService_Tunnel_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(TunnelServiceServer).Tunnel(&grpc.GenericServerStream[Frame, Frame]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type TunnelService_TunnelServer = grpc.BidiStreamingServer[Frame, Frame]
